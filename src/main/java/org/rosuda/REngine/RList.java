package org.rosuda.REngine;// org.rosuda.REngine.REngine library - Java client interface to R
...
public class RList extends Vector implements List {
    ...
    /** Note: sinde org.rosuda.REngine.RList is not really a Map, the returned set is only an approximation as it cannot reference duplicate or null names that may exist in the list */
    public Set keySet() {
	if (names==null) return null;
	...
	if (t==null) return;
	// NOTE: this if branch is dead since org.rosuda.REngine.RList cannot inherit from Map
	...
	public String toString() {
		return "org.rosuda.REngine.RList"+super.toString()+"{"+(isNamed()?"named,":"")+size()+"}";
	}
}
