...
public class REXPReference extends REXP {
	...
	/** create an external org.rosuda.REngine.REXP reference using given engine and handle. The handle value is just an (optional) identifier not used by the implementation directly. */
	...
	/** shortcut for <code>org.rosuda.REngine.REXPReference(eng, new Long(handle))</code> that is used by native code */
	REXPReference(REngine eng, long handle) {
		this(eng, Long.valueOf(handle));
	}

	/** resolve the external org.rosuda.REngine.REXP reference into an actual org.rosuda.REngine.REXP object. In addition, the value (if not <code>null</code>) will be cached for subsequent calls to <code>resolve</code> until <code>invalidate</code> is called. */
	public REXP resolve() {
		...
		try {
			...
			// this should never happen since we are org.rosuda.REngine.REXPReference
