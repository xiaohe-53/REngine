package org.rosuda.REngine;// environments are like REXPReferences except that they cannot be resolved

/** org.rosuda.REngine.REXPEnvironment represents an environment in R. Very much like {@link org.rosuda.REngine.REXPReference} this is a proxy object to the actual object on the R side. It provides methods for accessing the content of the environment. The actual implementation may vary by the back-end used and not all engines support environments. Check {@link org.rosuda.REngine.REngine.supportsEnvironments()} for the given engine. Environments are specific for a given engine, they cannot be passed across engines.
 */
public class REXPEnvironment extends REXP {
	/** engine associated with this environment */
	REngine eng;
	/** transparent handle that can be used by the engine to indentify the environment. It is not used by org.rosuda.REngine.REngine API itself. */
	Object handle;
	...
	public REXP get(String name, boolean resolve) throws REngineException {
		try {
			...
			throw(new REngineException(eng, "org.rosuda.REngine.REXPMismatchException:"+e+" in get()"));
		}
	}
	...
	public REXP parent(boolean resolve) throws REngineException {
		try {
			...
			throw(new REngineException(eng, "org.rosuda.REngine.REXPMismatchException:"+e+" in parent()"));
		}
	}
