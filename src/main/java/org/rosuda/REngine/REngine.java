...
/** org.rosuda.REngine.REngine is an abstract base class for all implementations of R engines. Subclasses can implement interfaces to R in many different ways.
...
public abstract class REngine {
    ...
    /** this is the designated constructor for org.rosuda.REngine.REngine classes. It uses reflection to call createEngine method on the given org.rosuda.REngine.REngine class.
	@param klass fully qualified class-name of a org.rosuda.REngine.REngine implementation
	@return org.rosuda.REngine.REngine implementation or <code>null</code> if <code>createEngine</code> invokation failed */
	...
	/** This is the extended constructor for org.rosuda.REngine.REngine classes. It uses reflection to call createEngine method on the given org.rosuda.REngine.REngine class with some additional control over the engine. Note that not all engines may support the extended version.
	 @param klass fully qualified class-name of a org.rosuda.REngine.REngine implementation
	 @param args arguments to pass to R for initialization
	 @param callbacks delegate for org.rosuda.REngine.REngine callbacks or <code>null</code> if callbacks won't be serviced (engine may not support callbacks)
	...
	/** parse a string into an expression vector
		...
		@param resolve resolve the resulting org.rosuda.REngine.REXP (<code>true</code>) or just return a reference (<code>false</code>)
	...
	/** fetch the contents of the given reference. The resulting org.rosuda.REngine.REXP may never be org.rosuda.REngine.REXPReference. The engine should raise a {@link #REngineException} exception if {@link #supportsReferences()} returns <code>false</code>.
	...
	/** removes reference from the R side. This method is called automatically by the finalizer of <code>org.rosuda.REngine.REXPReference</code> and should never be called directly.
	...
	//--- convenience methods --- (the org.rosuda.REngine.REXPMismatchException catches should be no-ops since the value type is guaranteed in the call to assign)
	/** convenience method equivalent to <code>assign(symbol, new org.rosuda.REngine.REXPDouble(d), null)</code> (see {@link #assign(String, REXP, REXP)})
	 @param symbol symbol name to assign to
	 @param d values to assign */
	public void assign(String symbol, double[] d) throws REngineException { try { assign(symbol, new REXPDouble(d), null); } catch (REXPMismatchException e) { throw(new REngineException(this, "org.rosuda.REngine.REXPMismatchException in assign(,double[]): "+e)); } }
	/** convenience method equivalent to <code>assign(symbol, new org.rosuda.REngine.REXPInteger(d), null)</code> (see {@link #assign(String, REXP, REXP)})
	 @param symbol symbol name to assign to
	 @param d values to assign */
	public void assign(String symbol, int[] d) throws REngineException { try { assign(symbol, new REXPInteger(d), null); } catch (REXPMismatchException e) { throw(new REngineException(this, "org.rosuda.REngine.REXPMismatchException in assign(,int[]): "+e)); } }
	/** convenience method equivalent to <code>assign(symbol, new org.rosuda.REngine.REXPString(d), null)</code> (see {@link #assign(String, REXP, REXP)})
	 @param symbol symbol name to assign to
	 @param d values to assign */
	public void assign(String symbol, String[] d) throws REngineException { try { assign(symbol, new REXPString(d), null); } catch (REXPMismatchException e) { throw(new REngineException(this, "org.rosuda.REngine.REXPMismatchException in assign(,String[]): "+e)); } }
	/** convenience method equivalent to <code>assign(symbol, new org.rosuda.REngine.REXPRaw(d), null)</code> (see {@link #assign(String, REXP, REXP)})
	 @param symbol symbol name to assign to
	 @param d values to assign */
	public void assign(String symbol, byte[] d) throws REngineException { try { assign(symbol, new REXPRaw(d), null); } catch (REXPMismatchException e) { throw(new REngineException(this, "org.rosuda.REngine.REXPMismatchException in assign(,byte[]): "+e)); } }
	/** convenience method equivalent to <code>assign(symbol, new org.rosuda.REngine.REXPString(d), null)</code> (see {@link #assign(String, REXP, REXP)})
	 @param symbol symbol name to assign to
